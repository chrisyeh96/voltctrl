...
Y = inv(X);
n = size(X,1);
simu_case = 1; % 1 - moderate load, large PV; 2 - heavy load, large PV; 3 - fluctuating generation
rng('default');
...
power_loss_weight = 0.1; % cost = reactive power cost + power_loss_weight*1/2*q'Xq
...
v_nocontrol = zeros(n,T); % voltage profile if there is no regulation
...
for t=1:T
    ...
    if(simu_case == 3)
        ...
        if(mod(t,flu_interval) ==1)
            G_fuctuating = G;
            G_fuctuating.bus(:,2) = G.bus(:,2).*(3*rand(n+1,1));
    ...
    tmp_case_mpc = case_mpc;
    pfresult_nocontrol = runpf(tmp_case_mpc);
    v_nocontrol(:,t) = (pfresult_nocontrol.bus(2:end,VM)).^2;
...
v_nocontrol_sqroot = sqrt(v_nocontrol)*G.basekV;
% if(simu_case<3)
%     number_of_plots = 3;
% else
%     number_of_plots=2;
...
if(simu_case==3)
    % plotting voltage profile without voltage regulation
    set(gca,'ColorOrderIndex',1);
    plot(v_nocontrol_sqroot(selected_buses,:)','-.','LineWidth',1);hold on;
...
if(simu_case == 3)
ylim([0.7*G.basekV,1.2*G.basekV]);
end
