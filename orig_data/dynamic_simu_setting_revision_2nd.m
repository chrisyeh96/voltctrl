    ...
    pq_fluc = zeros(n,2,T);
    pq_fluc(:,1,:) = -P_I(:,1:T)/1000 *1.1;
    pq_fluc(:,2,:) = -Q_I(:,1:T)/1000 *1.1;
    ...
    %PV_bus = [9,12,14,16,6,5,3,4,15,7,11,13,20,23,25,26,32,2]-1;
    actual_PV_profile = zeros(1,T);
    %PV_bus = [9,12,14,15,16,19,27,33,35,36,37,39,40,43,44,45,46,52]-1;
    %PV_bus = [2:19]-1;
    PV_bus = [9,12,14,16,19,10,11,13,15,7,2,4,20,23,25,26,32,8]-1;

    for(k=1:length(PV_bus))
        temp_vec = zeros(1,1,T);
        temp_vec(1,1,:) = PV_data(k,:)*7;
        %temp_vec(1,1,:) = PV_data(6,:)*5;
        actual_PV_profile = actual_PV_profile+transpose(squeeze(temp_vec(1,1,:)));
        pq_fluc(PV_bus(k),1,:) = pq_fluc(PV_bus(k),1,:) + temp_vec;
        %pq_fluc(PV_bus(k),2,:) = pq_fluc(PV_bus(k),2,:) + temp_vec/4;
    end
